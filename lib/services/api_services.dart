...
Future<List<Category>> getCategory() async {
  ...
  final response = await http.get(categoryUrl, headers: {"Authorization": "Bearer" + prefs.getString('spToken')});

  if (response.statusCode == 200){
    final jsonResponse = json.decode(response.body);
    return (jsonResponse['data'] as List).map((data) => Category.fromJson(data)).toList();
